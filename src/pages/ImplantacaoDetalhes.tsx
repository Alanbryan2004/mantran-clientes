...
export function ImplantacaoDetalhes() {
  ...
  return (
    <div className="space-y-6 w-full max-w-full pb-12">
              ...
              <div className="space-y-2">
                {visibleHistorico.map((item) => (
                  <div 
                    ...
                    className="flex items-center justify-between p-3.5 bg-slate-900/40 hover:bg-slate-800/70 rounded-xl border border-slate-800/80 hover:border-slate-700 group transition-all gap-3 cursor-pointer shadow-sm"
                    title="Clique para visualizar todas as informações deste registro"
                  >
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <p className="text-xs text-slate-200 font-medium truncate flex-1 group-hover:text-white transition-colors">
                        {item.texto}
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      {item.usuario_nome && (
                        <span 
                          className={`px-2.5 py-1 rounded-lg text-[11px] font-semibold flex items-center gap-1.5 border shadow-sm ${
                            (item.usuario_nome || '').toLowerCase().includes('cliente')
                              ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30'
                              : 'bg-slate-800/90 text-slate-300 border-slate-700/80'
                          }`} 
                          title={`Adicionado por ${item.usuario_nome}`}
                        >
                      ...
                      <button
                        ...
                        className="p-1.5 text-slate-400 hover:text-brand-300 hover:bg-slate-700/60 rounded-lg transition-colors"
      ...
      {/* Modal de Detalhes do Registro do Histórico */}
      <VisualizarHistoricoModal
        isOpen={!!selectedHistoricoItem}
