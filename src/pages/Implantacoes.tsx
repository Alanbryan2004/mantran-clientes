...
import { Plus, Rocket, ArrowRight, Trash2, Settings, ShoppingBag, Building } from 'lucide-react'
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {emAndamento.map(impl => {
                  ...
                  return (
                    <div 
                      key={impl.id} 
                      className="bg-dark-card border border-slate-800 rounded-xl p-5 hover:border-brand-500/50 transition-all duration-300 group cursor-pointer shadow-lg relative overflow-visible"
                    ...
                    >
                      ...
                      <div className="mt-3 pt-3 border-t border-slate-800/50 flex justify-between items-center text-sm text-slate-400 group-hover:text-slate-300">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {concluidos.map(impl => (
                  <div 
                    key={impl.id} 
                    className="bg-dark-card border border-slate-800 rounded-xl p-5 hover:border-green-500/30 transition-all duration-300 group cursor-pointer shadow-lg relative opacity-80 hover:opacity-100"
                    onClick={() => navigate(`/implantacoes/${impl.id}`)}
                  >
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-[10px] uppercase font-bold px-2 py-1 rounded-full bg-green-500/10 text-green-400 border border-green-500/20">
                        ✓ Concluído
                      </span>
                    </div>
